# ============================================================
#  gui.py
#  Interfaz gráfica del simulador — estilo videojuego unificado
...
NOMBRES = [
    ...
    "Joaqu Joaquín Silva", "Natalia Gómez", "Emilio Vargas", "Fernanda Castro", "Tomás Jiménez"
]
...
# ─────────────────────────────────────────────
#  WIDGET: Contenedor Modal Interno (Reemplaza Toplevel)
# ─────────────────────────────────────────────

class PanelModalInterno(tk.Frame):
    """Contenedor de interfaz integrado que flota en el centro del juego"""
    def __init__(self, parent, titulo: str, color_borde=COLOR_ACENTO, width=550, height=450):
        super().__init__(parent, bg=COLOR_FONDO, bd=2, relief="solid", highlightbackground=color_borde, highlightcolor=color_borde)
        self.place(relx=0.5, rely=0.5, anchor="center", width=width, height=height)
        
        # Guardamos la cabecera como un atributo de la clase (self.cabecera)
        ...
        tk.Label(
            self.cabecera, text=titulo, font=FUENTE_SUBTIT,
            bg=COLOR_PANEL, fg=color_borde, pady=10
        ).pack(side="left", padx=15)
        
        # Contenedor dinámico interno para los formularios
...
class PantallaBienvenida(tk.Frame):
    ...
    def _construir(self):
        tk.Label(
            ...
            font=("Courier New", 22, "bold"), bg=COLOR_FONDO, fg=COLOR_ACENTO
        ).pack(pady=(50, 4))

        tk.Label(
            self, text="UNIVERSITARIAS",
            font=("Courier New", 16, "bold"), bg=COLOR_FONDO, fg=COLOR_ACENTO
        ).pack(pady=(0, 6))
        ...
        tk.Label(
            self, text="¿Primera vez? No te preocupes — el juego te guía en cada paso.",
            font=FUENTE_PEQUEÑA, bg=COLOR_FONDO, fg=COLOR_TEXTO_SUAVE, pady=16
        ).pack()

        BotonJuego(self, texto="▶   COMENZAR JUEGO", comando=self.al_continuar, color_acento=COLOR_VERDE).pack(pady=10)
...
class PantallaConfiguracion(tk.Frame):
    ...
    def _construir(self):
        ...
        BotonJuego(self, texto="CONFIRMAR Y COMENZAR", comando=self._confirmar, color_acento=COLOR_VERDE).pack(pady=24)
    ...
    def _panel_servidores(self, parent):
        ...
        tk.Label(panel, text="🖥️  ¿CUÁNTOS SERVIDORES?", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_ACENTO, pady=10).pack()
        ...
        tk.Label(panel, text=ayuda, font=FUENTE_PEQUEÑA, bg=COLOR_PANEL, fg=COLOR_TEXTO, justify="left", padx=16).pack(anchor="w")

        frame_slider = tk.Frame(panel, bg=COLOR_PANEL)
        frame_slider.pack(fill="x", padx=16, pady=10)

        self.lbl_servidores = tk.Label(frame_slider, text="3", font=("Courier New", 22, "bold"), bg=COLOR_PANEL, fg=COLOR_VERDE)
        self.lbl_servidores.pack(side="right", padx=10)

        slider = tk.Scale(
            frame_slider, from_=1, to=5, orient="horizontal", variable=self.var_servidores,
            command=self._actualizar_lbl_servidores, bg=COLOR_PANEL, fg=COLOR_TEXTO,
            troughcolor=COLOR_BORDE, highlightthickness=0, activebackground=COLOR_ACENTO, length=200
        )
    ...
    def _panel_capacidad(self, parent):
        ...
        tk.Label(panel, text="¿CAPACIDAD POR SERVIDOR?", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_AMARILLO, pady=10).pack()
        ...
        self.lbl_capacidad = tk.Label(frame_slider, text="100", font=("Courier New", 22, "bold"), bg=COLOR_PANEL, fg=COLOR_AMARILLO)
        self.lbl_capacidad.pack(side="right", padx=10)

        slider = tk.Scale(
            frame_slider, from_=50, to=500, resolution=50, orient="horizontal", variable=self.var_capacidad,
            command=self._actualizar_lbl_capacidad, bg=COLOR_PANEL, fg=COLOR_TEXTO,
            troughcolor=COLOR_BORDE, highlightthickness=0, activebackground=COLOR_AMARILLO, length=200
        )
        slider.pack(side="left", fill="x", expand=True)
        tk.Frame(panel, bg=COLOR_BORDE, height=1).pack(fill="x", pady=(6, 0))
    ...
    def _panel_algoritmo(self, parent):
        ...
        tk.Label(panel, text="🔀  ALGORITMO DE BALANCEO", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_ROJO, pady=10).pack()

        tk.Label(panel, text="¿Cómo decides a qué servidor va cada estudiante?", font=FUENTE_PEQUEÑA, bg=COLOR_PANEL, fg=COLOR_TEXTO_SUAVE).pack(pady=(0, 10))
        ...
        for valor, nombre, desc in opciones:
            ...
            rb = tk.Radiobutton(
                f, text=nombre, variable=self.var_algoritmo, value=valor, font=FUENTE_SUBTIT,
                bg=COLOR_BOTON, fg=COLOR_TEXTO, selectcolor=COLOR_BORDE,
                activebackground=COLOR_BOTON_HOVER, activeforeground=COLOR_ACENTO, anchor="w"
            )
            rb.pack(fill="x", padx=10, pady=(8, 0))

            tk.Label(f, text=desc, font=FUENTE_PEQUEÑA, bg=COLOR_BOTON, fg=COLOR_TEXTO_SUAVE, justify="left", padx=30).pack(anchor="w", pady=(0, 8))
    ...
    def _confirmar(self):
        ...
        self.al_continuar(self.var_servidores.get(), self.var_capacidad.get(), algoritmo_map[self.var_algoritmo.get()])


# ─────────────────────────────────────────────
#  PANTALLA: Juego principal (Totalmente embebida)
# ─────────────────────────────────────────────
...
class PantallaJuego(tk.Frame):
    def __init__(self, parent, sistema: SistemaDeMatricula, al_finalizar):
        ...
        self.contador_id  = 1
        self.modal_activo: Optional[PanelModalInterno] = None
        self._construir()
    ...
    def _construir(self):
        ...
        contenedor_cabecera = tk.Frame(cabecera, bg=COLOR_PANEL)
        contenedor_cabecera.pack(fill="x", padx=20)

        tk.Label(
            contenedor_cabecera, text="🎓  PORTAL DE MATRÍCULAS — SALA DE CONTROL",
        ...
        self.btn_ayuda = tk.Button(
            contenedor_cabecera, text="?", font=("Courier New", 12, "bold"),
            bg=COLOR_BOTON, fg=COLOR_AMARILLO, activebackground=COLOR_BOTON_HOVER,
            activeforeground=COLOR_AMARILLO, relief="flat", bd=0, width=3, height=1,
            cursor="hand2", command=self._mostrar_ayuda_simulador
        )
        self.btn_ayuda.pack(side="right", padx=10, pady=10)
    ...
    def _destruir_modal_previo(self):
...
# GUIA DE SIMULADOR
    ...
    def _mostrar_ayuda_simulador(self):
        self._destruir_modal_previo()
        modal = PanelModalInterno(self, "GUÍA DEL SISTEMA OPERATIVO", COLOR_AMARILLO, width=650, height=520)
        self.modal_activo = modal

        # --- BOTÓN DE CIERRE EN LA CABECERA (Esquina superior derecha) ---
        btn_cerrar_top = tk.Button(
            modal.cabecera, text="×", font=("Courier New", 16, "bold"),
            bg=COLOR_PANEL, fg=COLOR_ROJO, activebackground=COLOR_PANEL,
            activeforeground=COLOR_TEXTO, relief="flat", bd=0, cursor="hand2",
            command=self._destruir_modal_previo
        )
        btn_cerrar_top.pack(side="right", padx=15, pady=5)

        guia = (
            "==================================================\n"
            "   GUÍA RÁPIDA DE OPERACIONES — SALA DE CONTROL\n"
            "==================================================\n\n"
            ...
            "   IMPORTANCIA: Permite probar el comportamiento de los\n"
            "   hilos de red y los algoritmos de planificación bajo estrés.\n\n"
            "3. ALGORITMO DE BALANCEO:\n"
            "   Define la lógica del proxy (puente dispositivo-internet) para distribuir las peticiones.\n"
            "   IMPORTANCIA: Un mal algoritmo causará que un servidor\n"
            "   colapse de forma aislada mientras los demás están libres.\n\n"
            "4. COLA DE ESPERA y FALLOS:\n"
            "   Maneja bloqueos de procesos y caídas de Hardware/Software.\n"
            "   IMPORTANCIA: Evalúa la tolerancia a fallos del sistema\n"
            "   y la resiliencia en alta disponibilidad."
        )
        
        text_area = tk.Text(modal.cuerpo, bg=COLOR_PANEL, fg=COLOR_TEXTO, font=FUENTE_NORMAL, relief="flat", wrap="word")
        text_area.pack(fill="both", expand=True, padx=5, pady=5)
        text_area.insert("1.0", guia)
    ...
    def _panel_servidores(self, parent):
        ...
        tk.Label(frame, text="ESTADO DE TUS SERVIDORES", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_ACENTO, pady=8).pack()
    ...
    def _panel_metricas(self, parent):
        ...
        tk.Label(frame, text="MÉTRICAS EN TIEMPO REAL", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_VERDE, pady=8).pack()
    ...
    def _metrica(self, parent, etiqueta, valor_inicial, color, col):
        ...
        tk.Label(f, text=valor_inicial, font=("Courier New", 18, "bold"), bg=COLOR_BOTON, fg=color).pack(pady=(8, 0))
    ...
    def _panel_acciones(self, parent):
        ...
        tk.Label(frame, text="ACCIONES DISPONIBLES", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_AMARILLO, pady=8).pack()

        acciones = [
            ("Enviar ola de estudiantes", "Simula que llegan estudiantes al portal.", self._accion_estudiantes, COLOR_ACENTO),
            ("Provocar un fallo", "Simula un fallo inesperado en hardware/software.", self._accion_fallo, COLOR_ROJO),
            ("Reparar servidor caído", "Vuelve a poner en línea un servidor afectado.", self._accion_reparar, COLOR_VERDE),
            ("Atender cola de espera", "Reintenta conectar a los alumnos retenidos.", self._accion_cola, COLOR_AMARILLO),
            ("Ver resultados finales", "Termina la simulación y calcula tu nota.", self.al_finalizar, COLOR_TEXTO_SUAVE),
        ]

        for texto, tooltip, comando, color in acciones:
            f = tk.Frame(frame, bg=COLOR_BOTON)
            f.pack(fill="x", padx=10, pady=3)
            BotonJuego(f, texto=texto, comando=comando, color_acento=color).pack(side="left", padx=(0, 8))
            tk.Label(f, text=tooltip, font=FUENTE_PEQUEÑA, bg=COLOR_BOTON, fg=COLOR_TEXTO_SUAVE, justify="left").pack(side="left", pady=6)

    def _panel_log(self, parent):
        frame = tk.Frame(parent, bg=COLOR_PANEL)
        frame.pack(fill="both", expand=True)
        tk.Frame(frame, bg=COLOR_TEXTO_SUAVE, height=2).pack(fill="x")
        tk.Label(frame, text="REGISTRO DE EVENTOS", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_TEXTO_SUAVE, pady=8).pack()

        self.log_text = tk.Text(frame, bg=COLOR_FONDO, fg=COLOR_TEXTO, font=FUENTE_PEQUEÑA, relief="flat", bd=0, state="disabled", wrap="word", height=10)
        self.log_text.pack(fill="both", expand=True, padx=8, pady=(0, 8))

    # ── ACCIONES CON FORMULARIOS INTEGRADOS (Cero Toplevels) ──
    ...
    def _accion_estudiantes(self):
        ...
        tk.Label(modal.cuerpo, text="¿Cuántos estudiantes intentan conectarse ahora?", font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO, pady=10).pack()
        
        var_cantidad = tk.IntVar(value=50)
        lbl_val = tk.Label(modal.cuerpo, text="50 estudiantes", font=("Courier New", 18, "bold"), bg=COLOR_FONDO, fg=COLOR_ACENTO)
        lbl_val.pack()

        def actualizar_label(val): lbl_val.configure(text=f"{val} estudiantes")

        tk.Scale(
            modal.cuerpo, from_=1, to=500, orient="horizontal", variable=var_cantidad, command=actualizar_label,
            bg=COLOR_FONDO, fg=COLOR_TEXTO, troughcolor=COLOR_BORDE, highlightthickness=0, activebackground=COLOR_ACENTO, length=320
        ).pack(pady=15)

        def ejecutar():
            self._procesar_ola(var_cantidad.get())
            self._destruir_modal_previo()

        BotonJuego(modal.cuerpo, "COMPROBAR TRÁFICO", ejecutar, color_acento=COLOR_VERDE).pack(pady=5)
        BotonJuego(modal, "CANCELAR", self._destruir_modal_previo, color_acento=COLOR_ROJO).pack(pady=(0, 10))
    ...
    def _accion_fallo(self):
        self._destruir_modal_previo()
        modal = PanelModalInterno(self, "SIMULAR UN FALLO DE RED", COLOR_ROJO, width=520, height=440)
        self.modal_activo = modal

        tk.Label(modal.cuerpo, text="Elige el tipo de percance y el nodo afectado:", font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO).pack(pady=5)

        var_tipo = tk.StringVar(value="1")
        var_srv  = tk.StringVar(value="0")

        frame_opciones = tk.Frame(modal.cuerpo, bg=COLOR_FONDO)
        frame_opciones.pack(fill="both", expand=True)

        # Tipos
        f_izq = tk.Frame(frame_opciones, bg=COLOR_FONDO)
        f_izq.pack(side="left", fill="both", expand=True)
        tk.Label(f_izq, text="Fallo:", font=FUENTE_SUBTIT, bg=COLOR_FONDO, fg=COLOR_ROJO).pack(anchor="w")
        for val, txt in [("1", "⚡ Sobrecarga"), ("2", "🔩 Hardware"), ("3", "🐛 Software"), ("4", "🦠 DDoS")]:
            tk.Radiobutton(f_izq, text=txt, variable=var_tipo, value=val, font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO, selectcolor=COLOR_BORDE).pack(anchor="w", py=2)

        # Servidores
        f_der = tk.Frame(frame_opciones, bg=COLOR_FONDO)
        f_der.pack(side="right", fill="both", expand=True)
        tk.Label(f_der, text="Dispositivo:", font=FUENTE_SUBTIT, bg=COLOR_FONDO, fg=COLOR_ROJO).pack(anchor="w")
        tk.Radiobutton(f_der, text="Aleatorio", variable=var_srv, value="0", font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO, selectcolor=COLOR_BORDE).pack(anchor="w")
        for srv in self.sistema.servidores:
            tk.Radiobutton(f_der, text=f"Servidor #{srv.id}", variable=var_srv, value=str(srv.id), font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO, selectcolor=COLOR_BORDE).pack(anchor="w")

        def ejecutar():
            tipo_map = {"1": TipoFallo.SOBRECARGA, "2": TipoFallo.HARDWARE, "3": TipoFallo.SOFTWARE, "4": TipoFallo.CIBERATAQUE}
            srv_id = None if var_srv.get() == "0" else int(var_srv.get())
            self.sistema.simular_fallo(tipo_map[var_tipo.get()], srv_id)
            self._actualizar_vista()
            self._destruir_modal_previo()

        BotonJuego(modal.cuerpo, "PROVOCAR CAÍDA", ejecutar, color_acento=COLOR_ROJO).pack(pady=5)
        BotonJuego(modal, "RETROCEDER", self._destruir_modal_previo, color_acento=COLOR_TEXTO_SUAVE).pack(pady=(0, 10))
    ...
    def _accion_reparar(self):
        ...
        if not caidos:
            modal = PanelModalInterno(self, "SISTEMA SALUDABLE", COLOR_VERDE, width=450, height=250)
            self.modal_activo = modal
            tk.Label(modal.cuerpo, text="Todos los servidores están estables.\nNo hay anomalías que requieran mantenimiento.", font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO, justify="center").pack(pady=20)
            BotonJuego(modal, "ENTENDIDO", self._destruir_modal_previo, color_acento=COLOR_VERDE).pack(pady=10)
            return

        modal = PanelModalInterno(self, "INFRAESTRUCTURA CAÍDA", COLOR_VERDE, width=480, height=320)
        self.modal_activo = modal
        tk.Label(modal.cuerpo, text="Selecciona el nodo a levantar:", font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO).pack(pady=10)

        var_srv = tk.StringVar(value=str(caidos[0].id))
        for srv in caidos:
            tk.Radiobutton(modal.cuerpo, text=f"Servidor #{srv.id} Inactivo", variable=var_srv, value=str(srv.id), font=FUENTE_SUBTIT, bg=COLOR_FONDO, fg=COLOR_ROJO, selectcolor=COLOR_BORDE).pack(anchor="w", padx=30, pady=4)
    ...
    def _accion_cola(self):
        ...
        if not self.sistema.cola_espera:
            modal = PanelModalInterno(self, "BUFFER DIRECTO", COLOR_VERDE, width=450, height=250)
            self.modal_activo = modal
            tk.Label(modal.cuerpo, text="La cola está vacía.\nNo hay solicitudes en espera en el balanceador.", font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO, justify="center").pack(pady=20)
            BotonJuego(modal, "VOLVER", self._destruir_modal_previo, color_acento=COLOR_VERDE).pack(pady=10)
            return
        ...
        tk.Label(modal.cuerpo, text=f"Hay {total} solicitudes en cola.\n¿Cuántas deseas despachar?", font=FUENTE_NORMAL, bg=COLOR_FONDO, fg=COLOR_TEXTO).pack(pady=10)

        var_cant = tk.IntVar(value=min(50, total))
        lbl_v = tk.Label(modal.cuerpo, text=f"{min(50, total)} estudiantes", font=("Courier New", 18, "bold"), bg=COLOR_FONDO, fg=COLOR_AMARILLO)
        lbl_v.pack()

        tk.Scale(modal.cuerpo, from_=1, to=total, orient="horizontal", variable=var_cant, command=lambda val: lbl_v.configure(text=f"{val} estudiantes"), bg=COLOR_FONDO, fg=COLOR_TEXTO, troughcolor=COLOR_BORDE, highlightthickness=0, activebackground=COLOR_AMARILLO, length=300).pack(pady=10)
        ...
        BotonJuego(modal, "CERRAR", self._destruir_modal_previo, color_acento=COLOR_ROJO).pack(pady=(0, 10))
    ...
    def _actualizar_servidores(self):
        for widget in self.frame_servidores_interno.winfo_children(): widget.destroy()
        for srv in self.sistema.servidores:
            f = tk.Frame(self.frame_servidores_interno, bg=COLOR_BOTON)
            f.pack(fill="x", pady=3)
            color_estado = COLOR_VERDE if srv.estado == EstadoServidor.ACTIVO else (COLOR_AMARILLO if srv.estado == EstadoServidor.SATURADO else COLOR_ROJO)
            tk.Label(f, text=f"{srv.estado.value}  Servidor #{srv.id}", font=FUENTE_SUBTIT, bg=COLOR_BOTON, fg=color_estado).pack(side="left", padx=10, pady=6)
            ...
            tk.Label(f, text=f"{srv.peticiones_actuales}/{srv.capacidad_maxima}", font=FUENTE_PEQUEÑA, bg=COLOR_BOTON, fg=COLOR_TEXTO_SUAVE).pack(side="left", padx=6)
            tk.Label(f, text=f"Procesadas: {srv.peticiones_procesadas}", font=FUENTE_PEQUEÑA, bg=COLOR_BOTON, fg=COLOR_TEXTO_SUAVE).pack(side="right", padx=10)
    ...
    def _actualizar_metricas(self):
        ...
        self.lbl_tasa.configure(text=f"{tasa:.1f}%", fg=COLOR_VERDE if tasa >= 80 else (COLOR_AMARILLO if tasa >= 50 else COLOR_ROJO))

    def _actualizar_log(self):
        self.log_text.configure(state="normal")
        self.log_text.delete("1.0", "end")
        for evento in self.sistema.log_eventos[-30:]: self.log_text.insert("end", evento + "\n")
    ...
    def _generar_estudiantes(self, cantidad: int) -> List[Estudiante]:
        ...
        for _ in range(cantidad):
            ...
            grupo.append(Estudiante(id=self.contador_id, nombre=nombre, prioridad=prioridad))
            self.contador_id += 1
        return grupo
...
class PantallaResultados(tk.Frame):
    ...
    def _construir(self):
        ...
        if tasa >= 95 and caidos == 0:
            calificacion = "🏆  INFRAESTRUCTURA PERFECTA"; color_cal = COLOR_VERDE; emoji_grande = "🏆"
        elif tasa >= 80:
            calificacion = "🥇  MUY BUENA"; color_cal = COLOR_VERDE; emoji_grande = "🥇"
        elif tasa >= 60:
            calificacion = "🥈  ACEPTABLE"; color_cal = COLOR_AMARILLO; emoji_grande = "🥈"
        elif tasa >= 40:
            calificacion = "🥉  DEFICIENTE"; color_cal = COLOR_ROJO; emoji_grande = "🥉"
        else:
            calificacion = "💀  COLAPSO TOTAL"; color_cal = COLOR_ROJO; emoji_grande = "💀"
        ...
        tk.Label(stats_frame, text="RESUMEN DE TU SIMULACIÓN", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_ACENTO, pady=10).pack()
        ...
        for etiqueta, valor, color in datos:
            ...
            tk.Label(fila, text=etiqueta + ":", font=FUENTE_NORMAL, bg=COLOR_PANEL, fg=COLOR_TEXTO_SUAVE, width=28, anchor="w").pack(side="left")
        ...
        tk.Label(stats_frame, text="LO QUE APRENDISTE HOY", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_AMARILLO, pady=6).pack()
        conclusiones = [
        ...
        for c in conclusiones: tk.Label(stats_frame, text=c, font=FUENTE_NORMAL, bg=COLOR_PANEL, fg=COLOR_TEXTO, anchor="w").pack(fill="x", padx=20, pady=1)

        if tasa < 60:
            tk.Frame(stats_frame, bg=COLOR_ROJO, height=1).pack(fill="x", pady=8)
            tk.Label(stats_frame, text="RECOMENDACIONES TÉCNICAS PARA MEJORAR", font=FUENTE_SUBTIT, bg=COLOR_PANEL, fg=COLOR_ROJO, pady=4).pack()
            recom_texto = "• Incrementa el número de servidores o la capacidad de procesos por nodo.\n"
            if "Round Robin" in self.sistema.balanceador.algoritmo.value: recom_texto += "• Round Robin puede saturar nodos específicos ante olas masivas; prueba con 'Menor Carga'.\n"
            recom_texto += "• Asegúrate de vaciar continuamente la cola de espera y reparar servidores caídos a tiempo."
            tk.Label(stats_frame, text=recom_texto, font=FUENTE_PEQUEÑA, bg=COLOR_PANEL, fg=COLOR_TEXTO, justify="left", anchor="w").pack(fill="x", padx=20, pady=(2, 6))
...
class Aplicacion:
    ...
    def _limpiar(self):
        if self._pantalla_actual: self._pantalla_actual.destroy()
